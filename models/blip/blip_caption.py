...
class BlipCaption(BlipBase):
    ...
    def sampling(
        ...
        while True:
            ...
            # 4. change the shape of next_tokens from (batch_size,) to (batch_size, 1) by adding
            # a new dimension
            # 5. use torch.cat to combine the current input_ids and the new tokens
            
            # Temperature + softmax + masking
            logits = next_tokens_logits / temperature
            topk_logits, topk_indices = torch.topk(logits, topk, dim=-1)
            topk_probs = F.softmax(topk_logits, dim=-1)
            next_tokens = torch.multinomial(topk_probs, num_samples=1)
            next_tokens = torch.gather(topk_indices, -1, next_tokens)
            input_ids = torch.cat([input_ids, next_tokens], dim=-1)
