...
def train_epoch(epoch, model, dataloader, optimizer, scheduler, loss_fn, device, temperature,
    ...
    for step, batch in enumerate(dataloader):
        ...
        sim_i2t = torch.matmul(image_feat, text_feat.T) / temperature
        sim_t2i = sim_i2t.T

        # Define one-hot targets
        targets = torch.eye(sim_i2t.size(0), device=device, dtype=torch.float32)

        # Compute cross-entropy losses
        loss_i2t = loss_fn(sim_i2t, targets.argmax(dim=1))
        loss_t2i = loss_fn(sim_t2i, targets.argmax(dim=1))

        # Compute final loss as weighted average
